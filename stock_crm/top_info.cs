...
namespace stock_crm
{
    public partial class top_info : customForm
    {
        ...
        private string query_top_produit = "SELECT p.id, p.nom, SUM(cp.quantite) AS quantite, SUM(cp.idproduit) AS NbCommandes, cp.idcommande, SUM(c.montant) AS MontantTotal FROM produit p, commandeproduit cp, commande c WHERE p.id = cp.idproduit AND c.id = cp.idcommande GROUP BY idproduit ORDER BY NbCommandes DESC";
        ...
        private void nb_cmd_Click(object sender, EventArgs e)
        {
           ...
           // string date = DateTime.Today.ToString("yyyy-MM-dd H:mm:ss");
            ...
            string str_date = date.ToString("yyyy-MM-dd H:mm:ss");
            string str_date_inter = intervalle.ToString("yyyy-MM-dd H:mm:ss");
